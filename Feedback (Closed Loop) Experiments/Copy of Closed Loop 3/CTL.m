...
% Target set points
xtarget{1} = 5; % Can Define multiple target values for the system
xtarget{2} = 5;
...
% Don't worry about this, it gets taken care of down below... the first
% move is always to do nothing (you need to take a measurement so you can
...
for i = 1:Iter
    ...
    if mod(time(i), step*ccycle) == 0
        ...
        for sys = 1:length(xtarget)
            ...
            if i == 1
                control{sys}(i) = 0;
                disp(sprintf('This is Time 0... Do not do anything unless I tell you',sys,time(i)))
            else
                if control{sys}(i) == 0
    ...
    for sys = 1:length(xtarget)
        ...
        if meas_up == 1
            if i == 1
                x_est{sys}(:,i) = [0.5;meandata(sys,mcount);meandata(sys,mcount);meandata(sys,mcount)];
            else
    ...
    % Progress the approximate state according to new control input
    %close all
    ddd = [{'-b*'}, {'-b*'}, {'-b*'},{'-r*'}];
    for sys = 1:length(xtarget)
        U{sys} = control{sys}(i);
